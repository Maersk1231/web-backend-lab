...
def review_to_wordlist(review, remove_stopwords=False):
    ...
    if remove_stopwords:
        # 自定义停用词列表，保留否定词和情感词
        stops = set(stopwords.words("english")) - set(['not', 'no', 'never', 'very', 'so', 'too', 'good', 'bad', 'great', 'terrible'])
...
def main():
    ...
    # 加载Word2Vec特征
    word2vec_features_file = "word2vec_features_final.npz"
    ...
    output.to_csv("Word2Vec_LR_Final.csv", index=False, quoting=3)
    print("最终优化版Word2Vec+逻辑回归提交文件已生成: Word2Vec_LR_Final.csv")
    print("任务完成！")
